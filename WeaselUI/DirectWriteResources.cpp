...
vector<string> c_split(const char* in, const char* delim) {
  std::regex re{delim};
  return vector<string>{std::cregex_token_iterator(in, in + strlen(in), re, -1),
                        std::cregex_token_iterator()};
}

vector<wstring> wc_split(const wchar_t* in, const wchar_t* delim) {
  std::wregex re{delim};
  return vector<wstring>{
...
HRESULT DirectWriteResources::InitResources(const wstring& label_font_face,
  ...
  auto func = [&](const wstring& fontface, int fontpoint,
    ...
    // ensure valid fontpoint value
    fontpoint = (fontpoint <= 0) ? 16 : fontpoint;
    HR(pDWFactory->CreateTextFormat(
        _mainFontFace.c_str(), NULL, fontWeight, fontStyle,
        DWRITE_FONT_STRETCH_NORMAL, fontpoint * dpiScaleFontPoint, L"",
  ...
  func(font_face, font_point, pPreeditTextFormat, wrapping_preedit);
  func(label_font_face, label_font_point, pLabelTextFormat, wrapping);
  func(comment_font_face, comment_font_point, pCommentTextFormat, wrapping);
  return S_OK;
}
